...
class EnterpriseAccountController extends \App\Http\Controllers\Controller
{
    ...
    public function index()
    {
        ...
        // NEED TO FIX!!
        $companies = Company::select('company.id','company.name')
            //->join('insurance_company', 'insurance_company.company_id', '=', 'company.id')
        ...
        /* Get Transfers */
        $transfers = CheckinLedger::select('locations.name as company','locations.club_id', 'ledger.active_date', 'locations.address', 'programs.name', 'ledger.total', 'ledger.stripe_status', 'ledger.visit_count', 'ledger.visit_process_count','ledger.reimbursement', 'ledger.company_id')
    ...
    public function downloadInsurance() {
        ...
        $whereDate = " WHERE EXTRACT(YEAR FROM active_date) = '{$year}'";
        ...
        $whereCompany = '';
        // if ($selectedCompany > 0) $whereCompany = ' AND ledger_member.company_id = '.$selectedCompany;
        ...
        $insurances = DB::select(
            ...
            FROM ledger_member
                JOIN member_program ON member_program.user_id = ledger_member.member_id AND member_program.program_id = ledger_member.program_id
                 JOIN locations on locations.id = ledger_member.location_id AND (ledger_member.program_id IN (34,35))  $whereDate $whereCompany

                 UNION

           select confirmation_id,TO_CHAR(active_date, 'YYYYMM') AS activity_date,count,club_id from _ledger_report_activity_detail LEFT JOIN member_program ON member_program.user_id = _ledger_report_activity_detail.user_id
            AND member_program.program_id = _ledger_report_activity_detail.program_id
             $whereDate $whereCompany
        ");
