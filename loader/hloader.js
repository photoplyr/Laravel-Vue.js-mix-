...
function main() {
                ...
                ppool.query("select * from humana", function(err, rows) {
                            ...
                            if (rows.rows.length == 0) {
                                ...
                                process.exit(1);
                                    ...
                                    for (let row of rows.rows) {
                                                ...
                                                var sql = "select * from public.user where LOWER(fname) = LOWER('" + row.fname + "') and LOWER(lname) = LOWER('" + row.lname + "') and company_id = 34";
                                                console.log(sql);
                                             ...
                                             ppool.query(sql, function(err, person) {
                                                 ...
                                                 if (person.rows.length > 0) {
                                                        ...
                                                        var a = "insert into member_program (status,user_id,program_id,membership) values (1," + person.rows[0].id + "," + program_id + ",'"  + code + "')  ON CONFLICT(status,user_id,program_id,membership) DO NOTHING;";
